...
- [x] Step 4: When (if) the session raises goal.md reconciliation, approve the minimal wording edit via a fresh relay (`maintainer9`); capture the exchange verbatim. If it never raises it, note the absence.
- [x] Step 5: If the session stalls > 5 min without observable progress, send the proven T6 status-ping text via a fresh neutral relay (one budgeted ping).
