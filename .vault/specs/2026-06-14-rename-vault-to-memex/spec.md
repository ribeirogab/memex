...
- [ ] `bash .agents/skills/memex-link/tests/run.sh` exits 0 (PASS). The script's fixture rename and directory references flipped consistently.
...
- [ ] Scaffold byte-equivalence post-rename: `diff -r .agents/skills/memex-link/ skills/memex/scaffold/skills/memex-link/` accounts only for the known fixture-dirname difference (`.memex` vs `memex`), matching the pre-rename baseline diff. And `diff -r .agents/skills/memex-link/ plugins/memex/skills/link/` returns empty.
