...
- [ ] **AC-8** Non-goals hold: `git diff --name-only origin/main...HEAD` (or `git diff --name-only` of the branch's changes) lists **neither** `.memex/scripts/validate-spec.sh` **nor** `skills/memex/scaffold/vault-scripts/validate-spec.sh`, and lists no file under `.memex/learnings/` and no file under `.memex/specs/` outside `2026-06-18-memex-worktrees/`.
