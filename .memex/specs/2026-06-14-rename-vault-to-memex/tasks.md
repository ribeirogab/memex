...
- [x] **Step 1: Save cross-copy diff baselines + confirm green tests**
...
- [x] **Step 1: Rename the tracked directory with history**

```bash
git mv .vault .memex
```

- [x] **Step 2: Remove the orphaned leftover holding the gitignored `.obsidian/`**
...
- [x] **Step 3: Flip the `.gitignore` Obsidian path**
...
- [x] **Step 4: Verify history followed the rename**
...
- [x] **Step 1: Flip canon + indices + templates + conventions**
...
- [x] **Step 2: Flip learnings, excluding the historical-narrative survivor**
...
- [x] **Step 3: Verify — only the survivor retains `.vault` in learnings; canon clean**
...
- [x] **Step 2: Verify empty (incl. that `vault-files.md` kept its name, only contents flipped)**
...
- [x] **Step 1: Flip the three companion-skill copies (brainstorming, code-review, new-pr, recall, writing-plans)**
...
- [x] **Step 2: Flip the plugin commands**

```bash
perl -pi -e 's/\.vault/.memex/g' plugins/memex/commands/*.md
```

- [x] **Step 3: Verify empty across companions + commands (link handled in Task 6)**
...
- [x] **Step 1: Rename the fixture directories with history (preserve dot/no-dot convention per copy)**
...
- [x] **Step 2: Flip script + expected + fixture contents in all three copies**
...
- [x] **Step 3: Verify the two working test copies stay green**
...
- [x] **Step 4: Verify scaffold copy behavior unchanged (still pre-broken FATAL, not a new failure mode)**

```bash
bash skills/memex/scaffold/skills/memex-link/tests/run.sh 2>&1 | head -1   # expect: FATAL: .memex/ not found ...
```

- [x] **Step 5: Verify no new cross-copy divergence vs baseline**
...
- [x] **Step 6: Verify no stray `.vault` left in link copies**
...
- [x] **Step 1: Global anchored sweep — every `.vault` survivor must be justified**
...
- [x] **Step 2: Phrase-artifact check (redundant "the `.memex/` vault")**
...
- [x] **Step 3: Confirm directory inventory**
...
- [x] **Step 4: Run the memex validation checks**
...
- [x] **Step 5: Tick the spec acceptance criteria**
